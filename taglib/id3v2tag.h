...
namespace TagLib
{
    ...
    namespace ID3v2
    {
         ...
         *
         * \note Writing non-ISO-8859-1 tags is not implemented intentionally. Use UTF-16 or UTF-8 instead.
         *
        ...
        public:
             ...
             *
             * \note You should be able to ignore the \a factory parameter in almost all situations.  You would want to specify your own FrameFactory subclass in the case that you are extending TagLib to support additional frame types, which would be incorperated into your factory.
             *
             * \see FrameFactory
             */
             ...
             *
             * \code TagLib::MPEG::File f("foo.mp3");
             *
             ...
             *
             *   if(!l.isEmpty()) std::cout << l.front()->toString() << std::endl;
             * }
             *
             * \endcode
             *
            ...
            /*!
             * Returns the frame list for frames with the id \a frameID or an empty list if there are no frames of that type.  This is just a convenience and is equivalent to:
             *
             * \code frameListMap()[frameID];
             * \endcode
             *
             * \see frameListMap()
             */
            ...
            /*!
             * Remove a frame from the tag.  If \a del is true the frame's memory will be freed; if it is false, it must be deleted by the user.
             *
             * \note Using this method will invalidate any pointers on the list returned by frameList()
             */
             ...
             *  - if the frame ID is "TIPL" (involved peoples list), and if all the roles defined in the frame are known in TextIdentificationFrame::involvedPeopleMap(), then "<role>=<name>" will be contained in the returned obejct for each
             *  - if the frame ID is "TMCL" (musician credit list), then
             *    "PERFORMER:<instrument>=<name>" will be contained in the returned PropertyMap for each defined musician In any other case, the unsupportedData() of the returned object will contain the frame's ID and, in case of a frame ID which is allowed to appear more than once, the description, separated by a "/".
             *
             */
             ...
             * \a properties must be taken from properties().unsupportedData(); they are of one of the following forms:
             ...
             * Render the tag back to binary data, suitable to be written to disk.
             *
             * The \a version parameter specifies the version of the rendered ID3v2 tag. It can be either 4 or 3.
             */
            ...
            /*!
             * Sets the string handler that decides how the "Latin-1" data will be converted to and from binary data. If the parameter \a handler is null, the previous handler is released and default ISO-8859-1 handler is restored.
             *
        ...
        protected:
            ...
            /*!
             * Sets the value of the text frame with the Frame ID \a id to \a value. If the frame does not exist, it is created.
             */
