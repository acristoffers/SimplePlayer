...
namespace TagLib
{
    ...
    public:
        ...
        /*!
         * Removes unsupported properties, or a subset of them, from the tag. The parameter \a properties must contain only entries from properties().unsupportedData(). BIC: Will become virtual in future releases. Currently the non-virtual standard implementation of TagLib::Tag does nothing, since there are no unsupported elements.
         */
        void removeUnsupportedProperties(const StringList &properties);

        /*!
         * Sets the tags of this File to those specified in \a properties. This default implementation sets only the tags for which setter methods exist in this class (artist, album, ...), and only one value per key; the rest will be contained in the returned PropertyMap.
         */
        ...
        /*!
         * Sets the genre to \a s.  If \a s is String::null then this value will be cleared.  For tag formats that use a fixed set of genres, the appropriate value will be selected based on a string comparison.  A list of available genres for those formats should be available in that type's implementation.
         */
         ...
         *
         * \note This will no affect any of the lower level details of the tag.  For instance if any of the tag type specific data (maybe a URL for a band) is set, this will not modify or copy that.  This just copies using the API in this class.
         *
         * If \a overwrite is true then the values will be unconditionally copied. If false only empty values will be overwritten.
         */
