...
namespace TagLib
{
    namespace ID3v2
    {
         ...
         *
         * This implements both <i>abstract factory</i> and <i>singleton</i> patterns of which more information is available on the web and in software design textbooks (Notably <i>Design Patters</i>).
         *
         * \note You do not need to use this factory to create new frames to add to an ID3v2::Tag.  You can instantiate frame subclasses directly (with new) and add them to a tag using ID3v2::Tag::addFrame()
         *
        ...
        public:
            ...
            /*!
             * Create a frame based on \a data.  \a synchSafeInts should only be set false if we are parsing an old tag (v2.3 or older) that does not support synchsafe ints.
             *
             * \deprecated Please use the method below that accepts a ID3v2::Header instance in new code.
             */
            Frame *createFrame(const ByteVector &data, bool synchSafeInts) const;

            /*!
             * Create a frame based on \a data.  \a version should indicate the ID3v2 version of the tag.  As ID3v2.4 is the most current version of the standard 4 is the default.
             *
             * \deprecated Please use the method below that accepts a ID3v2::Header instance in new code.
             */
             ...
             * \a encoding.  If no value is set the frames with either default to the encoding type that was parsed and new frames default to Latin1.
             *
        ...
        private:
             ...
             * \a to.  If the frame matches the \a from pattern and converts the frame ID in the \a header or simply does nothing if the frame ID does not match.
             */
