...
namespace TagLib
{
    namespace Ogg
    {
        ...
        public:
            ...
            /*!
             * Pack \a packets into Ogg pages using the \a strategy for pagination. The page number indicater inside of the rendered packets will start with \a firstPage and be incremented for each page rendered.
             * \a containsLastPacket should be set to true if \a packets contains the last page in the stream and will set the appropriate flag in the last rendered Ogg page's header.  \a streamSerialNumber should be set to the serial number for this stream.
             *
             * \note The "absolute granule position" is currently always zeroed using this method as this suffices for the comment headers.
             *
             * \warning The pages returned by this method must be deleted by the user. You can use List<T>::setAutoDelete(true) to set these pages to be automatically deleted when this list passes out of scope.
             *
