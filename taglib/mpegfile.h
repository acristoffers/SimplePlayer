...
namespace TagLib
{
    ...
    namespace MPEG
    {
        ...
        public:
            ...
            /*!
             * Constructs an MPEG file from \a file.  If \a readProperties is true the file's audio properties will also be read.
             *
            ...
            /*!
             * Constructs an MPEG file from \a stream.  If \a readProperties is true the file's audio properties will also be read.
             *
             ...
             *
             * \note As this tag is not implemented as an ID3v2 tag or an ID3v1 tag, but a union of the two this pointer may not be cast to the specific tag types.
             *
             * \see ID3v1Tag()
             * \see ID3v2Tag()
             * \see APETag()
             ...
             * Save the file.  This will attempt to save all of the tag types that are specified by OR-ing together TagTypes values.  The save() method above uses AllTags.  This returns true if saving was successful.
             *
             * If \a stripOthers is true this strips all tags not included in the mask, but does not modify them in memory, so later calls to save() which make use of these tags will remain valid.  This also strips empty tags.
             */
             ...
             * If \a stripOthers is true this strips all tags not included in the mask, but does not modify them in memory, so later calls to save() which make use of these tags will remain valid.  This also strips empty tags.
             *
             * The \a id3v2Version parameter specifies the version of the saved ID3v2 tag. It can be either 4 or 3.
             */
            // BIC: combine with the above method
             ...
             *
             * If \a create is false (the default) this may return a null pointer if there is no valid ID3v2 tag.  If \a create is true it will create an ID3v2 tag if one does not exist and returns a valid pointer.
             *
             ...
             *
             * If \a create is false (the default) this may return a null pointer if there is no valid ID3v1 tag.  If \a create is true it will create an ID3v1 tag if one does not exist and returns a valid pointer.
             *
             ...
             *
             * If \a create is false (the default) this may return a null pointer if there is no valid APE tag.  If \a create is true it will create an APE tag if one does not exist and returns a valid pointer.
             *
        ...
        private:
            ...
            /*!
             * MPEG frames can be recognized by the bit pattern 11111111 111, so the first byte is easy to check for, however checking to see if the second byte starts with \e 111 is a bit more tricky, hence this member function.
             */
